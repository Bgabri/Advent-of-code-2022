    ...
    class Day13 {
        static function main() {
            trace("solution to part 1: " + part1(loadFile("inputs/13.txt")));
            trace("solution to part 2: " + part2(loadFile("inputs/13.txt")));    
        }
        ...
        static function part1(input:Array<Dynamic>):Int {
            ...
            for (i in 0...Std.int(input.length/2)) {
                var left  = input[2*i];
                var right = input[2*i+1];
                if (valid(left, right)== Ordered) total += i+1;
            }

            return total;
        }

        static function valid(left:Dynamic, right:Dynamic):Order {
            // trace(left, right);
            var ordered = false;

            var leftInt  = left.length == null;
            var rightInt = right.length == null;

            switch ([leftInt, rightInt]) {
                case [true, true]:
                    if (left < right) return Ordered;
                    else if (left > right) return Unorderd;
                    return Unknown;
                case [false, true]: return valid(left, [right]);
                case [true, false]: return valid([left], right);

                case _:
            }
            

            var l = Std.int(Math.max(right.length, left.length));

            for (i in 0...l) {
                if (right.length <= i) return Unorderd;
                if (left.length  <= i) return Ordered;
                var v = valid(left[i], right[i]);
        ...
        static function part2(input:Array<Dynamic>):Int {
            ...
            input.sort((a,b) -> valid(a,b) == Ordered ? -1 : 1);
            // if (valid(a,b) == Ordered) 1 else -1);

            var total = 1;
            for (i=>inp in input) {
                if (inp == _2 || inp == _6) total *= i+1;
            }
        ...
        static function loadFile(file:String):Array<Dynamic> {
            ...
            var input:Array<String> = ["[[[[2],9,1,[2,2,4,8]],[1,8,8],9,[7,2,[7,0,1],0,[10,9,10,3]]]]",
        ...
        static function rParse(s:String):Dynamic {
            ...
            var b = 0;
            var sA = s.split(",");
            var iA = sA.map(v -> Std.parseInt(v));
            ...
            for (code in s) {
                ...
                switch (char) {
                    ...
                    case ",": 
                        if (b == 0) {
                            a.push(rParse(newS.substring(0, newS.length-1)));
                            newS = "";
                        }
                    case _:
                }
